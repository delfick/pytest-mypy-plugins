...
class MypyExecutor:
    ...
    def execute(self, mypy_cmd_options: List[str]) -> Tuple[int, Tuple[str, str]]:
        ...
        if self.same_process:
            return self.typecheck_in_same_process(mypy_cmd_options)
        else:
            return self.typecheck_in_new_subprocess(mypy_cmd_options)

    def typecheck_in_new_subprocess(self, mypy_cmd_options: List[Any]) -> Tuple[int, Tuple[str, str]]:
    ...
    def typecheck_in_same_process(self, mypy_cmd_options: List[Any]) -> Tuple[int, Tuple[str, str]]:
        return_code = -1
...
class Runner:
    def __init__(
        ...
        *,
        ...
        base_pyproject_toml_fpath: Path,
    ) -> None:
        self.files = files
        self.config = config
        ...
        self.base_pyproject_toml_fpath = base_pyproject_toml_fpath
        ...
        try:
            ...
            with utils.cd(execution_path):
                ...
                Runner(
                    ...
                    base_pyproject_toml_fpath=self.base_pyproject_toml_fpath,
                ).run()
        finally:
            temp_dir.cleanup()
